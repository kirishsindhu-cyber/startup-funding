...
with col1:
    if 'IndustryVertical' in df.columns and 'AmountInUSD' in df.columns:
        ...
        plt.tight_layout()
        st.pyplot(fig1)
        plt.savefig('output/top_sectors.png', dpi=150, bbox_inches='tight')
        plt.close()
...
with col3:
    if 'InvestorsName' in df.columns:
        ...
        plt.tight_layout()
        st.pyplot(fig3)
        plt.savefig('output/top_investors.png', dpi=150, bbox_inches='tight')
        plt.close()
...
with col5:
    if 'Year' in df.columns:
        ...
        plt.tight_layout()
        st.pyplot(fig5)
        plt.savefig('output/yearwise_deals.png', dpi=150, bbox_inches='tight')
        plt.close()
...
with col6:
    if 'Year' in df.columns and 'AmountInUSD' in df.columns:
        ...
        plt.tight_layout()
        st.pyplot(fig6)
        plt.savefig('output/yearwise_funding.png', dpi=150, bbox_inches='tight')
        plt.close()
